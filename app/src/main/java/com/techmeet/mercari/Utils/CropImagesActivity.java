...
public class CropImagesActivity extends AppCompatActivity {
    ...
    private void SubmitAction() {
        ...
        if(ContextCompat.checkSelfPermission(CropImagesActivity.this, Manifest.permission.WRITE_EXTERNAL_STORAGE)== PackageManager.PERMISSION_GRANTED) {
            ...
            for(Bitmap bitmap : croppedImagesBitmap){
                Uri uri = SaveImage.addToFav(CropImagesActivity.this, "Upload", System.currentTimeMillis() +"", bitmap);
            ...
            ActivityCompat.requestPermissions(CropImagesActivity.this, new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE}, Constants.EXTERNAL_STORAGE_PERMISSION);
        }
    }
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        if (requestCode == Constants.EXTERNAL_STORAGE_PERMISSION) {
            if (grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                SubmitAction();
            } else {
                Toast.makeText(getApplicationContext(), "External storage permission denied", Toast.LENGTH_SHORT).show();
            }
        }
    }
}